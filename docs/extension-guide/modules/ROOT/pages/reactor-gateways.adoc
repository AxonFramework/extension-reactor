...
Axon provides a notion of xref:axon-framework-references:messaging-concepts:message-intercepting.adoc[interceptors]. The Reactor gateways allow for similar interceptor logic, namely the `ReactorMessageDispatchInterceptor` and `ReactorResultHandlerInterceptor`.
